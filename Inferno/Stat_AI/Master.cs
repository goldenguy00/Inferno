        ...
        public static void MasterChanges(CharacterMaster master)
        {
            if (master.teamIndex == TeamIndex.Monster || master.teamIndex == TeamIndex.Void)
            {
                ...
                if (master.name != "GolemMaster(Clone)")
                {
                    ...
                    BaseAI.aimVectorDampTime = Mathf.Max(0.001f, 0.031f - (0.0001f * Main.AIScaling.Value * Run.instance.stageClearCount));
                    BaseAI.aimVectorMaxSpeed = 250f + (20f * Main.AIScaling.Value * Run.instance.stageClearCount);
                    BaseAI.enemyAttentionDuration = 1.5f;
                }
                else
                {
                    BaseAI.fullVision = true;
                    BaseAI.aimVectorDampTime = Mathf.Max(0.001f, 0.09f - (0.0003f * Main.AIScaling.Value * Run.instance.stageClearCount));
                    BaseAI.aimVectorMaxSpeed = 250f + (20f * Main.AIScaling.Value * Run.instance.stageClearCount);
                    BaseAI.enemyAttentionDuration = 1.5f;
                }
                ...
                switch (master.name)
                {
                    ...
                    case "BisonMaster(Clone)":
                        AISkillDriver BisonCharge = (from x in master.GetComponents<AISkillDriver>()
                                                     where x.skillSlot == SkillSlot.Utility
                                                     select x).First();
